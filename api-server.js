...
// Crear nueva sala
app.post('/api/admin/salas', (req, res) => {
    try {
        const { nombre, tipo, descripcion, capacidad, imagen, disponible } = req.body;
        ...
        const nuevaSala = {
            ...
            equipamiento: ['WiFi', 'Proyector', 'Aire acondicionado']
        };
...
// Actualizar sala
app.put('/api/admin/salas/:id', (req, res) => {
    try {
        const salaId = parseInt(req.params.id);
        const { nombre, tipo, descripcion, capacidad, imagen, disponible } = req.body;
        ...
        // Actualizar campos
        if (nombre) salas[salaIndex].nombre = nombre;
        if (tipo) salas[salaIndex].tipo = tipo;
        if (descripcion) salas[salaIndex].descripcion = descripcion;
        if (capacidad) salas[salaIndex].capacidad = parseInt(capacidad);
        if (imagen) salas[salaIndex].imagen = imagen;
        if (disponible !== undefined) salas[salaIndex].disponible = disponible;
...
// Eliminar sala
app.delete('/api/admin/salas/:id', (req, res) => {
    try {
        const salaId = parseInt(req.params.id);
        const salaIndex = salas.findIndex(s => s.id === salaId);
